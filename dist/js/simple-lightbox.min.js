  ...
  st.fn.simpleLightbox = function(N) {
    ...
    function p() {
      ...
      var t = 'pid=' + (V + 1),
        e = o.href.split('#')[0] + '#' + t;
      s
        ? history[f ? 'replaceState' : 'pushState']('', E.title, e)
        : f
        ? o.replace(e)
        : (o.hash = t),
        (f = !0);
    ...
    var h,
      e,
      H = 'ontouchstart' in ot,
      L = (ot.navigator.pointerEnabled || ot.navigator.msPointerEnabled, 0),
      j = 0,
      K = st(),
      i = function() {
        ...
        if (K.length) {
          ...
          (l.src = K.attr('src')),
            ...
            st(l).on('error', function(t) {
              ...
              v === V && e
                ? it()
                : (N.alertError && alert(N.alertErrorMessage), nt(e ? o : 1));
            }),
            (l.onload = function() {
              void 0 !== o &&
                $.eq(V)
                  ...
                  .trigger(
                    st.Event(
                      (1 === o ? 'nextDone' : 'prevDone') + '.simplelightbox'
                    )
                  ),
      ...
      C = function() {
        ...
        _.on('touchstart.' + G + ' mousedown.' + G, function(t) {
          ...
          .on('touchmove.' + G + ' mousemove.' + G + ' MSPointerMove', function(
            t
          ) {
            if (!A) return !0;
            if (
            ...
            function(t) {
              if (
                ...
                H &&
                  ...
                  (0 === (I = t.touches.length)
                    ? (z(W, x, b),
                      ...
                      (R = !1))
                    : 1 === I
                    ? ((c = t.touches[0].clientX), (p = t.touches[0].clientY))
                    : 1 < I && (u = null)),
                A)
              ) {
                var e = !(A = !1);
                N.loop ||
                  (0 === V && L < 0 && (e = !1),
                  V >= $.length - 1 && 0 < L && (e = !1)),
                  Math.abs(L) > N.swipeTolerance && e
                    ? nt(0 < L ? 1 : -1)
                    : N.animationSlide &&
                      (B
                        ? et(N.animationSpeed / 1e3, '0px')
                        : _.animate({ left: O + 'px' }, N.animationSpeed / 2)),
                  N.swipeClose &&
                    50 < Math.abs(j) &&
                    Math.abs(L) < N.swipeTolerance &&
                    it();
              }
            }
          )
      ...
      w = function() {
        ...
        st('<img />')
          .attr('src', $.eq(t).attr(N.sourceAttr))
          .on('load', function() {
            -1 == d.indexOf(st(this).attr('src')) &&
              d.push(st(this).attr('src')),
              $.eq(V).trigger(st.Event('nextImageLoaded.simplelightbox'));
          }),
          st('<img />')
            .attr('src', $.eq(e).attr(N.sourceAttr))
            .on('load', function() {
              -1 == d.indexOf(st(this).attr('src')) &&
                d.push(st(this).attr('src')),
      ...
      nt = function(e) {
        ...
        if (!(m || ((t < 0 || t >= $.length) && !1 === N.loop))) {
          ...
          N.animationSlide &&
            (B
              ? et(N.animationSpeed / 1e3, -100 * e - L + 'px')
              : (a.left =
      ...
      it = function() {
        if (!m) {
          ...
          t.trigger(st.Event('close.simplelightbox')),
            N.history &&
              (s
                ? history.pushState('', E.title, o.pathname + o.search)
                : (o.hash = ''),
              clearTimeout(h)),
            st(
            ...
            (i.className = 'sl-scrollbar-measure'),
              ...
              0 < e &&
                st('body')
                  ...
                  .css({ 'padding-right': s + e });
          }
        } else
          st('body')
            .removeClass('hidden-scroll')
            .css({ 'padding-right': st('body').data('padding') });
        return e;
      };
    return (
      N.close && r.appendTo(b),
      N.showCounter &&
        1 < $.length &&
        (c.appendTo(b), c.find('.sl-total').text($.length)),
      N.nav && J.appendTo(b),
      ...
      N.disableRightClick &&
        st(E).on('contextmenu', '.sl-image img', function(t) {
          return !1;
        }),
      N.enableKeyboard &&
        st(E).on(
